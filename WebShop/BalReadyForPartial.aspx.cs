...
namespace WebShop
{
  public partial class BalReadyForPartial : System.Web.UI.Page
  {
    ...
    public static List<SalesOrderClass> GetCustomers()
    {
      ...
      {
        using (SqlCommand cmd = new SqlCommand("exec  WS_CustOrderList_details @t_sect='BALR' , @t_ofbp = '" + r_ofbp + "'"))
        {
          cmd.Connection = con;
          ...
          using (SqlDataReader sdr = cmd.ExecuteReader())
          {
            while (sdr.Read())
            {
              SalesOrders.Add(new SalesOrderClass
              {
                ...
                partial_despatch = sdr["partial_despatch"].ToString(),
    ...
    public static List<SalesOrderClass> GetFOCOrders()
    {
      ...
      using (SqlConnection con = new SqlConnection(constr))
      {
        using (SqlCommand cmd = new SqlCommand("exec  WS_CustOrderList_details_foc @t_sect='BALR' , @t_ofbp = '" + r_ofbp + "'"))
        {
          cmd.Connection = con;
          List<SalesOrderClass> SalesOrders = new List<SalesOrderClass>();
          con.Open();
    ...
    public class SalesOrderClass
    {
      ...
      public string partial_despatch { get; set; }
